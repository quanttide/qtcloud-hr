...
- 收件箱同步时保留人工审核结果和招聘流程状态，避免已通过候选人再次回到待处理。

## [0.1.16] - 2026-09-08

### Fixed

- 识别没有文件扩展名但 MIME 类型为 Word 的简历，并为其补齐 `.doc` 或 `.docx` 文件名。

## [0.1.15] - 2026-09-08

### Fixed

- 允许 FC HTTP 触发器转发 `PATCH` 请求，使招聘页面的人工审核“通过/拒绝”状态可以保存。

## [0.1.14] - 2026-09-07

### Fixed

- 持久化候选人快照时保留简历附件内部 ID，重启或多实例刷新后仍可生成预览和下载链接。

## [0.1.13] - 2026-09-07

### Fixed

- 禁止包含当前认证账号编号的 provider 状态响应被缓存。
- 增加 Bearer 认证主体与伪造操作员头的隔离测试。

## [0.1.12] - 2026-09-07

### Added

- provider 状态接口向当前已认证用户返回账号编号，便于配置招聘真实动作写权限。
